...
/* Global full distance matrix (set in main for reuse). */
static const double *g_full_dist = NULL;
static int g_full_m = 0;
static const ExData *g_full_ex = NULL;
...
static int map_start_id(const ExData *ex, const double start_rad[2]);
static int map_end_id(const ExData *ex, const double end_rad[2]);
...
static void mark_dirty_for_segment(int seg, int port_count, int *dirty) {
  if (!dirty) return;
  if (seg > 0) dirty[seg - 1] = 1;
  if (seg < port_count) dirty[seg] = 1;
}

static void segment_endpoints(const ExData *ex, const Visit *visits, int n_visits,
                              const int *seg_end, int seg_count, int seg,
                              double start_rad[2], double end_rad[2]) {
  if (seg == 0) {
    start_rad[0] = ex->LatLonRad[0];
    start_rad[1] = ex->LatLonRad[1];
  } else {
    int start_idx = seg_end[seg - 1];
    int start_ex = visits[start_idx].ex_idx;
    start_rad[0] = ex->LatLonRad[start_ex * 4 + 0];
    start_rad[1] = ex->LatLonRad[start_ex * 4 + 1];
  }

  if (seg < seg_count - 1) {
    int end_idx = seg_end[seg];
    int end_ex = visits[end_idx].ex_idx;
    end_rad[0] = ex->LatLonRad[end_ex * 4 + 0];
    end_rad[1] = ex->LatLonRad[end_ex * 4 + 1];
  } else {
    end_rad[0] = ex->LatLonRad[2];
    end_rad[1] = ex->LatLonRad[3];
  }
...
static int mutate_inject_closest_station(Visit **visits_io, int *n_visits_io,
                                         const ExData *ex, const double *dist, int Size,
                                         double ship_cap, int left_seg,
                                         GRBenv *seg_env, double seg_timelimit,
                                         int *out_src_seg, int *out_dst_seg,
                                         int *out_stat_ex, double *out_delta) {
  if (!visits_io || !*visits_io || !n_visits_io || *n_visits_io <= 0) return 0;
  if (!seg_env) return 0;
  ...
  if (left_seg < 0 || left_seg >= port_count) return 0;
  ...
  int left_count = seg_station_count[left_seg];
  int right_count = seg_station_count[right_seg];
  if (left_count == 0 && right_count == 0) {
    free(seg_first); free(seg_end); free(seg_station_count); free(seg_amount);
    free(seg_of_idx); free(seg_station_offsets); free(seg_station_ex); free(seg_station_pos);
    return 0;
  }
  ...
  for (int i = 0; i < n_visits; i++) {
    if (visits[i].type != tSTAT) continue;
    int s = seg_of_idx[i];
    if (s == left_seg || s == right_seg) continue;
    int ex_i = visits[i].ex_idx;

    double d_left = INF;
    if (left_count > 0) {
      int off = seg_station_offsets[left_seg];
      for (int j = 0; j < left_count; j++) {
        double d = min_pair_dist(dist, 2 * Size, ex_i, seg_station_ex[off + j]);
        if (d < d_left) d_left = d;
      }
    }
    double d_right = INF;
    if (right_count > 0) {
      int off = seg_station_offsets[right_seg];
      for (int j = 0; j < right_count; j++) {
        double d = min_pair_dist(dist, 2 * Size, ex_i, seg_station_ex[off + j]);
        if (d < d_right) d_right = d;
      }
    }
    if (d_left >= INF * 0.5 && d_right >= INF * 0.5) continue;

    int dst = (d_left <= d_right) ? left_seg : right_seg;
    double dmin = (d_left <= d_right) ? d_left : d_right;
    if (seg_amount[dst] + ex->Amount[ex_i] > ship_cap) continue;
  ...
  int stat_ex = visits[best_idx].ex_idx;
  int donor_seg = best_src_seg;
  int dst_seg = best_dst_seg;

  double left_start[2], left_end[2];
  double right_start[2], right_end[2];
  double donor_start[2], donor_end[2];
  segment_endpoints(ex, visits, n_visits, seg_end, seg_count, left_seg, left_start, left_end);
  segment_endpoints(ex, visits, n_visits, seg_end, seg_count, right_seg, right_start, right_end);
  segment_endpoints(ex, visits, n_visits, seg_end, seg_count, donor_seg, donor_start, donor_end);

  double d_left_old = 0.0;
  double d_right_old = 0.0;
  double d_donor_old = 0.0;
  if (solve_segment_distance(seg_env, ex,
                             seg_station_ex + seg_station_offsets[left_seg], left_count,
                             left_start, left_end, seg_timelimit, &d_left_old, NULL, NULL) != 0 ||
      solve_segment_distance(seg_env, ex,
                             seg_station_ex + seg_station_offsets[right_seg], right_count,
                             right_start, right_end, seg_timelimit, &d_right_old, NULL, NULL) != 0 ||
      solve_segment_distance(seg_env, ex,
                             seg_station_ex + seg_station_offsets[donor_seg], seg_station_count[donor_seg],
                             donor_start, donor_end, seg_timelimit, &d_donor_old, NULL, NULL) != 0 ||
      d_left_old >= 100000.0 || d_right_old >= 100000.0 || d_donor_old >= 100000.0) {
    free(seg_first); free(seg_end); free(seg_station_count); free(seg_amount);
    free(seg_of_idx); free(seg_station_offsets); free(seg_station_ex); free(seg_station_pos);
    return 0;
  }
  double baseline = d_left_old + d_right_old + d_donor_old;

  int left_new_count = left_count + (dst_seg == left_seg ? 1 : 0);
  int right_new_count = right_count + (dst_seg == right_seg ? 1 : 0);
  int donor_new_count = seg_station_count[donor_seg] - 1;

  int *left_new = (int*)xmalloc((size_t)(left_new_count > 0 ? left_new_count : 1) * sizeof(int));
  int *right_new = (int*)xmalloc((size_t)(right_new_count > 0 ? right_new_count : 1) * sizeof(int));
  int *donor_new = (int*)xmalloc((size_t)(donor_new_count > 0 ? donor_new_count : 1) * sizeof(int));

  int left_pos = 0;
  int right_pos = 0;
  int donor_pos = 0;
  for (int i = 0; i < left_count; i++) left_new[left_pos++] = seg_station_ex[seg_station_offsets[left_seg] + i];
  for (int i = 0; i < right_count; i++) right_new[right_pos++] = seg_station_ex[seg_station_offsets[right_seg] + i];
  int donor_off = seg_station_offsets[donor_seg];
  for (int i = 0; i < seg_station_count[donor_seg]; i++) {
    int ex_i = seg_station_ex[donor_off + i];
    if (ex_i == stat_ex) continue;
    donor_new[donor_pos++] = ex_i;
  }
  if (dst_seg == left_seg) left_new[left_pos++] = stat_ex;
  if (dst_seg == right_seg) right_new[right_pos++] = stat_ex;

  double d_left_new = 0.0;
  double d_right_new = 0.0;
  double d_donor_new = 0.0;
  int err_left = solve_segment_distance(seg_env, ex, left_new, left_pos,
                                        left_start, left_end, seg_timelimit, &d_left_new, NULL, NULL);
  int err_right = solve_segment_distance(seg_env, ex, right_new, right_pos,
                                         right_start, right_end, seg_timelimit, &d_right_new, NULL, NULL);
  int err_donor = solve_segment_distance(seg_env, ex, donor_new, donor_pos,
                                         donor_start, donor_end, seg_timelimit, &d_donor_new, NULL, NULL);

  free(left_new);
  free(right_new);
  free(donor_new);

  if (err_left || err_right || err_donor ||
      d_left_new >= 100000.0 || d_right_new >= 100000.0 || d_donor_new >= 100000.0) {
    free(seg_first); free(seg_end); free(seg_station_count); free(seg_amount);
    free(seg_of_idx); free(seg_station_offsets); free(seg_station_ex); free(seg_station_pos);
    return 0;
  }

  double new_total = d_left_new + d_right_new + d_donor_new;
  if (new_total >= baseline - 1e-6) {
    free(seg_first); free(seg_end); free(seg_station_count); free(seg_amount);
    free(seg_of_idx); free(seg_station_offsets); free(seg_station_ex); free(seg_station_pos);
    return 0;
  }
  ...
  int insert_idx = 0;
  if (dst_seg < seg_count - 1) insert_idx = seg_end[dst_seg];
  else insert_idx = n_visits;
  int from_idx = best_idx;
  ...
  if (out_src_seg) *out_src_seg = donor_seg;
  if (out_dst_seg) *out_dst_seg = dst_seg;
  if (out_stat_ex) *out_stat_ex = stat_ex;
  if (out_delta) *out_delta = new_total - baseline;
    ...
    if (mutate_inject_closest_station(visits_io, n_visits_io, ex, dist, Size,
      ...
      if (out_mut_note && out_mut_note_len > 0) {
        snprintf(out_mut_note, out_mut_note_len,
                 ...
                 stat_ex, src_seg + 1, dst_seg + 1, delta);
      }
    }
  }
...
static int map_start_id(const ExData *ex, const double start_rad[2]) {
  if (!ex || !start_rad) return -1;
  if (start_rad[0] == ex->LatLonRad[0] && start_rad[1] == ex->LatLonRad[1]) {
    return 0; /* ship entry */
  }
  for (int i = 0; i < ex->Size; i++) {
    if (start_rad[0] == ex->LatLonRad[i*4 + 0] &&
        start_rad[1] == ex->LatLonRad[i*4 + 1]) {
      return 2 * i;
    }
  }
  return -1;
...
static int map_end_id(const ExData *ex, const double end_rad[2]) {
  if (!ex || !end_rad) return -1;
  if (end_rad[0] == ex->LatLonRad[2] && end_rad[1] == ex->LatLonRad[3]) {
    return 1; /* ship exit */
  }
  for (int i = 0; i < ex->Size; i++) {
    if (end_rad[0] == ex->LatLonRad[i*4 + 0] &&
        end_rad[1] == ex->LatLonRad[i*4 + 1]) {
      return 2 * i;
    }
  }
  return -1;
...
static int solve_segment_distance(GRBenv *env, const ExData *orig,
  ...
  int start_id = -1;
  int end_id = -1;
  if (g_full_dist && g_full_ex == orig && g_full_m > 0) {
    start_id = map_start_id(orig, start_rad);
    end_id = map_end_id(orig, end_rad);
  }

  if (start_id >= 0 && end_id >= 0) {
    if (n_station == 0) {
      if (out_dist) *out_dist = g_full_dist[start_id * g_full_m + end_id];
      if (out_order) *out_order = NULL;
      if (out_order_len) *out_order_len = 0;
      return 0;
    }

    int seg_size = 1 + n_station;
    int n = 2 * seg_size;
    double *dist = (double*)xmalloc((size_t)n * (size_t)n * sizeof(double));
    int *entry = (int*)xmalloc((size_t)seg_size * sizeof(int));
    int *exit = (int*)xmalloc((size_t)seg_size * sizeof(int));
    entry[0] = start_id;
    exit[0] = end_id;
    ...
    for (int i = 0; i < seg_size; i++) {
      ...
      for (int j = 0; j < seg_size; j++) {
        int gj0 = entry[j];
        int gj1 = exit[j];
        dist[(2*i + 0) * n + (2*j + 0)] = g_full_dist[gi0 * g_full_m + gj0];
        dist[(2*i + 0) * n + (2*j + 1)] = g_full_dist[gi0 * g_full_m + gj1];
        dist[(2*i + 1) * n + (2*j + 0)] = g_full_dist[gi1 * g_full_m + gj0];
        dist[(2*i + 1) * n + (2*j + 1)] = g_full_dist[gi1 * g_full_m + gj1];
      }
    }
    ...
    int *node_tour = NULL;
    int node_len = 0;
    int error = solve_tsp_distance(env, dist, seg_size, timelimit, 0, out_dist,
    ...
    if (!error && out_order && out_order_len && node_tour) {
      ...
      int letour_len = 0;
      int *letour = node_tour_to_letour(node_tour, node_len, seg_size, &letour_len);
