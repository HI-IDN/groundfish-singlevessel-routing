/*
  ...
  solve with SCIP (lazy subtour cuts via constraint handler).

  Build (macOS, example):
    clang main_scip.c \
      $(scip-config --cflags --libs) \
      -lm \
      -o demo_scip

  Run:
    ./demo_scip data/data2023spring.dat 1 [timelimit_seconds]
*/
...
typedef struct SCIP_ConshdlrData CONSHDLRDATA;

static int find_subtour_len(SCIP* scip, SCIP_SOL* sol, CONSHDLRDATA* data, int* tour) {
  int n = data->n;
  double *vals = (double*)xmalloc((size_t)n * (size_t)n * sizeof(double));
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) {
      vals[i*n + j] = SCIPgetSolVal(scip, sol, data->xvars[i*n + j]);
    }
  }
  int len = 0;
  findsubtour_directed(n, vals, &len, tour);
  free(vals);
  return len;
}

static SCIP_RETCODE enforce_subtour_cuts(SCIP* scip, SCIP_SOL* sol, CONSHDLRDATA* data, SCIP_RESULT* result) {
  int n = data->n;
  double *vals = (double*)xmalloc((size_t)n * (size_t)n * sizeof(double));
  int is_integral = 1;
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) {
      double v = SCIPgetSolVal(scip, sol, data->xvars[i*n + j]);
      vals[i*n + j] = v;
      if (!SCIPisFeasIntegral(scip, v)) is_integral = 0;
    }
  }
  ...
  if (len < n) {
    char cname[64];
    SCIP_CONS *cons = NULL;
    sprintf(cname, "subtour_cut_%d", data->cutcount++);
    SCIP_RETCODE ret = SCIPcreateConsBasicLinear(
        scip, &cons, cname, 0, NULL, NULL, -SCIPinfinity(scip), (double)len - 1.0);
    if (ret != SCIP_OKAY) {
      free(tour);
      return ret;
    }
    for (int a = 0; a < len; a++) {
      for (int b = a + 1; b < len; b++) {
        int i = tour[a];
        int j = tour[b];
        SCIPaddCoefLinear(scip, cons, data->xvars[i*n + j], 1.0);
        SCIPaddCoefLinear(scip, cons, data->xvars[j*n + i], 1.0);
      }
    }
...
static SCIP_DECL_CONSENFOLP(consEnfolpSubtour) {
  (void)conss;
  (void)nconss;
  (void)nusefulconss;
  (void)solinfeasible;
  ...
  return enforce_subtour_cuts(scip, NULL, data, result);
}

static SCIP_DECL_CONSENFOPS(consEnfopsSubtour) {
  (void)conss;
  (void)nconss;
  (void)nusefulconss;
  (void)solinfeasible;
...
static SCIP_DECL_CONSCHECK(consCheckSubtour) {
  (void)conss;
  (void)nconss;
  ...
  int n = data->n;
  int *tour = (int*)xmalloc((size_t)n * sizeof(int));
  int len = find_subtour_len(scip, sol, data, tour);
  if (len < n) *result = SCIP_INFEASIBLE;
  else *result = SCIP_FEASIBLE;
  free(tour);
  return SCIP_OKAY;
}

static SCIP_DECL_CONSLOCK(consLockSubtour) {
  (void)scip;
  (void)conshdlr;
  (void)cons;
  (void)locktype;
...
static SCIP_DECL_CONSFREE(consFreeSubtour) {
  CONSHDLRDATA *data = (CONSHDLRDATA*)SCIPconshdlrGetData(conshdlr);
  if (data) free(data);
  return SCIP_OKAY;
}
...
/* no callback: MTZ constraints enforce a single tour */
...
int main(int argc, char **argv) {
  ...
  /* Create SCIP model (full NxN vars + MTZ subtour elimination) */
  SCIP *scip = NULL;
  SCIP_VAR **xvars = NULL;
  SCIP_CONS **cons_out = NULL;
  SCIP_CONS **cons_in = NULL;
  SCIP_CONS **cons_pair = NULL;
  int scip_error = 0;

#define SCIP_TRY(x) do { \
  SCIP_RETCODE _ret = (x); \
  if (_ret != SCIP_OKAY) { \
    fprintf(stderr, "SCIP error %d at %s:%d\n", _ret, __FILE__, __LINE__); \
    scip_error = 1; \
    goto SCIP_QUIT; \
  } \
} while (0)
  ...
  SCIP_TRY(SCIPincludeDefaultPlugins(scip));
  SCIP_TRY(SCIPcreateProbBasic(scip, "planner"));
  SCIP_TRY(SCIPsetObjsense(scip, SCIP_OBJSENSE_MINIMIZE));
  SCIP_TRY(SCIPsetRealParam(scip, "limits/time", timelimit));
  SCIP_TRY(SCIPsetIntParam(scip, "parallel/maxnthreads", 4));
  int scip_threads = 0;
  SCIP_TRY(SCIPgetIntParam(scip, "parallel/maxnthreads", &scip_threads));
  printf("SCIP threads: %d\n", scip_threads);

  /* vars e[i,j] index = i*n + j */
  xvars = (SCIP_VAR**)xcalloc((size_t)n * (size_t)n, sizeof(SCIP_VAR*));
  const double scip_inf = SCIPinfinity(scip);
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) {
      char vname[64];
      double ub = (i == j) ? 0.0 : 1.0;
      double obj = dist[i*n + j];
      if (ub == 0.0) obj = 0.0;
      if (obj >= scip_inf) obj = scip_inf * 0.1;
      sprintf(vname, "e_%d_%d", i, j);
      SCIP_TRY(SCIPcreateVarBasic(scip, &xvars[i*n + j], vname, 0.0, ub,
                                  obj, SCIP_VARTYPE_BINARY));
      SCIP_TRY(SCIPaddVar(scip, xvars[i*n + j]));
    }
  }

  /* outdegree=1 and indegree=1 */
  cons_out = (SCIP_CONS**)xcalloc((size_t)n, sizeof(SCIP_CONS*));
  cons_in = (SCIP_CONS**)xcalloc((size_t)n, sizeof(SCIP_CONS*));
  for (int i = 0; i < n; i++) {
    char cname[64];
    sprintf(cname, "out_%d", i);
    SCIP_TRY(SCIPcreateConsBasicLinear(scip, &cons_out[i], cname, 0, NULL, NULL, 1.0, 1.0));
    for (int j = 0; j < n; j++) {
      SCIP_TRY(SCIPaddCoefLinear(scip, cons_out[i], xvars[i*n + j], 1.0));
    }
    SCIP_TRY(SCIPaddCons(scip, cons_out[i]));
  }
  for (int j = 0; j < n; j++) {
    char cname[64];
    sprintf(cname, "in_%d", j);
    SCIP_TRY(SCIPcreateConsBasicLinear(scip, &cons_in[j], cname, 0, NULL, NULL, 1.0, 1.0));
    for (int i = 0; i < n; i++) {
      SCIP_TRY(SCIPaddCoefLinear(scip, cons_in[j], xvars[i*n + j], 1.0));
    }
    SCIP_TRY(SCIPaddCons(scip, cons_in[j]));
  }

  /* internal constraint: e[2i,2i+1]+e[2i+1,2i]==1 */
  cons_pair = (SCIP_CONS**)xcalloc((size_t)Size, sizeof(SCIP_CONS*));
  for (int i = 0; i < Size; i++) {
    char cname[64];
    int a = 2*i, b = 2*i+1;
    sprintf(cname, "pair_%d", i);
    SCIP_TRY(SCIPcreateConsBasicLinear(scip, &cons_pair[i], cname, 0, NULL, NULL, 1.0, 1.0));
    SCIP_TRY(SCIPaddCoefLinear(scip, cons_pair[i], xvars[a*n + b], 1.0));
    SCIP_TRY(SCIPaddCoefLinear(scip, cons_pair[i], xvars[b*n + a], 1.0));
    SCIP_TRY(SCIPaddCons(scip, cons_pair[i]));
  }

  /* subtour constraint handler (lazy cuts) */
  CONSHDLRDATA *chdata = (CONSHDLRDATA*)xcalloc(1, sizeof(CONSHDLRDATA));
  chdata->n = n;
  chdata->xvars = xvars;
  chdata->cutcount = 0;

  SCIP_CONSHDLR *conshdlr = NULL;
  SCIP_TRY(SCIPincludeConshdlrBasic(
      scip, &conshdlr, "subtour", "subtour elimination",
      0, 0, 0, FALSE,
      consEnfolpSubtour, consEnfopsSubtour, consCheckSubtour, consLockSubtour, chdata));
  SCIP_TRY(SCIPsetConshdlrFree(scip, conshdlr, consFreeSubtour));

  SCIP_CONS *basecons = NULL;
  SCIP_TRY(SCIPcreateCons(scip, &basecons, "subtour_base", conshdlr, NULL,
                          FALSE, FALSE, TRUE, TRUE, FALSE,
                          FALSE, FALSE, TRUE, TRUE, FALSE));
  SCIP_TRY(SCIPaddCons(scip, basecons));
  SCIP_TRY(SCIPreleaseCons(scip, &basecons));

  /* Do not forbid land-crossing arcs; distances already reroute via waypoints. */

  SCIP_TRY(SCIPsolve(scip));
  ...
  if (bestsol) {
    ...
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
        sol[i*n + j] = SCIPgetSolVal(scip, bestsol, xvars[i*n + j]);
      }
    }
...
SCIP_QUIT:
  if (scip) {
    if (cons_pair) {
      for (int i = 0; i < Size; i++) {
        if (cons_pair[i]) SCIPreleaseCons(scip, &cons_pair[i]);
      }
    }
    if (cons_in) {
      for (int i = 0; i < n; i++) {
        if (cons_in[i]) SCIPreleaseCons(scip, &cons_in[i]);
      }
    }
    if (cons_out) {
      for (int i = 0; i < n; i++) {
        if (cons_out[i]) SCIPreleaseCons(scip, &cons_out[i]);
      }
    }
    if (xvars) {
      for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
          if (xvars[i*n + j]) SCIPreleaseVar(scip, &xvars[i*n + j]);
        }
      }
    }
    SCIPfree(&scip);
