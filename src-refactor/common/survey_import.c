/*
 * Historical survey station import using DataSet.
 */
...
int main(int argc, char **argv) {
    ...
    if (!dat_file || !db_path) {
        fprintf(stderr, "Usage: %s <historical_survey.dat> <gsp_data.db>\n", argv[0]);
        return 1;
    }
...

    {
        ...
        sqlite3_bind_text(meta_stmt, 1, "survey_source_file", -1, SQLITE_STATIC);
        ...
        sqlite3_bind_text(meta_stmt, 1, "survey_import_time", -1, SQLITE_STATIC);
    ...
    printf("Imported historical survey stations\n");
    printf("  source:   %s\n", dat_file);
    printf("  database: %s\n", db_path);
    printf("  stations: %d\n", dataset.n_stations);
    printf("  note: survey and distances were cleared; run the distance step next\n");
