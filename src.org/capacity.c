...
static void print_active_ports(const ItemVec *items, const ExData *ex) {
  int total = 0;
  for (int i = 0; i < ex->Size; i++) {
    if (ex->Type[i] == tPORT) total++;
  }
  if (total == 0) {
    printf("Active ports: none\n");
    return;
  }

  char **names = (char**)xcalloc((size_t)total, sizeof(char*));
  int *counts = (int*)xcalloc((size_t)total, sizeof(int));
  int uniq = 0;

  for (int i = 0; i < ex->Size; i++) {
    if (ex->Type[i] != tPORT) continue;
    const Item *it = &items->a[ex->ItemIndex[i]];
    const char *name = it->Name ? it->Name : "PORT";
    int found = -1;
    for (int j = 0; j < uniq; j++) {
      if (strcmp(names[j], name) == 0) { found = j; break; }
    }
    if (found >= 0) {
      counts[found] += 1;
    } else {
      names[uniq] = (char*)name;
      counts[uniq] = 1;
      uniq++;
    }
  }

  printf("Active ports (%d nodes):", total);
  for (int i = 0; i < uniq; i++) {
    printf(" %s", names[i]);
    if (counts[i] > 1) printf("(%d)", counts[i]);
  }
  printf("\n");
  free(names);
  free(counts);
...
int main(int argc, char **argv) {
  if (argc < 3) {
    fprintf(stderr, "Usage: %s <datafile.dat> <ship_id 1..4> [timelimit_seconds] [--time-limit <sec>] [--write-dat <out.dat>]\n", argv[0]);
    return 1;
  }
  ...
  for (int i = 3; i < argc; i++) {
    if (strcmp(argv[i], "--no-ports") == 0) {
      fprintf(stderr, "capacity.c ignores --no-ports (ports reset cargo)\n");
    ...
    } else if (strncmp(argv[i], "--write-dat=", 12) == 0) {
      write_dat = argv[i] + 12;
  ...
  GRBsetintparam(GRBgetenv(model), "OutputFlag", verbose ? 1 : 0);
  GRBsetintparam(GRBgetenv(model), "LogToConsole", verbose ? 1 : 0);
