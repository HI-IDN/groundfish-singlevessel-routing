...
The fixed-port model solves a capacity-constrained MIP with predefined port visit positions.
Port candidates are the ports actually visited during the 2023 spring survey, stored in
`dat/candidate_ports.json`. These fixed waypoints define the port schedule; the MIP then
optimises station assignment and ordering within each segment subject to capacity constraints.
